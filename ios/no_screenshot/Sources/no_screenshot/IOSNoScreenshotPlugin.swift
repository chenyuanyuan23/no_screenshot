...
public class IOSNoScreenshotPlugin: NSObject, FlutterPlugin, FlutterStreamHandler {
    ...
    public func applicationWillResignActive(_ application: UIApplication) {
        if Self.hasPresentedSafariViewController() { return }
    ...
    public func applicationDidBecomeActive(_ application: UIApplication) {
        if Self.hasPresentedSafariViewController() {
            Self.observeSafariDismiss { [weak self] in
                self?.fetchPersistedState()
            }
            return
        }
    ...
    private static func hasPresentedSafariViewController() -> Bool {
        var activeWindow: UIWindow?
        if #available(iOS 13.0, *) {
        ...
        guard let rootVC = activeWindow?.rootViewController else { return false }
        var vc = rootVC
        while let presented = vc.presentedViewController {
            if presented is SFSafariViewController {
                return true
            }
            vc = presented
        }
    ...
    private static func observeSafariDismiss(completion: @escaping () -> Void) {
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) {
            if self.hasPresentedSafariViewController() {
                self.observeSafariDismiss(completion: completion)
            } else {
                completion()
            }
        }
